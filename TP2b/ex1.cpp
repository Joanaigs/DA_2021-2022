...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    for(int i=0; i<vp.size(); i++){
        for(int j=i+1; j<vp.size(); j++){
            double temp=sqrt(pow(vp[i].x-vp[j].x, 2)+pow(vp[i].y-vp[j].y, 2));
            if(temp<res.dmin){
                res.p1=vp[i];
                res.p2=vp[j];
                res.dmin=temp;
...
Result stripClosest(std::vector<Point> &vp, int size, Result d)
{
    Result min = d; // Initialize the minimum distance as d

    sortByY(vp, 0, vp.size());

    // Pick all points one by one and try the next points till the difference
    // between y coordinates is smaller than d.
    // This is a proven fact that this loop runs at most 6 times
    for (int i = 0; i < size; ++i)
        for (int j = i+1; j < size && (vp[j].y - vp[i].y) < min.dmin; ++j)
            if (dist(vp[i],vp[j]) < min.dmin) {
                min.dmin = dist(vp[i], vp[j]);
                min.p1=vp[i];
                min.p2=vp[2];
            }
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    ...
    int mid = vp.size()/2;
    Point midPoint = vp[mid];

    // Consider the vertical line passing
    // through the middle point calculate
    // the smallest distance dl on left
    // of middle point and dr on right side
    std::vector<Point> vl={vp.begin(), vp.begin()+mid};
    Result dl = nearestPoints_DC(vl);
    std::vector<Point> vr={vp.begin()+mid, vp.end()};
    Result dr = nearestPoints_DC(vr);
    ...
    int j = 0;
    for (int i = 0; i < vp.size(); i++)
        if (abs(vp[i].x - midPoint.x) < d.dmin)
            temp[j] = vp[i], j++;

    // Find the closest points in strip.
    // Return the minimum of d and closest
    // distance is strip[]
    if(d.dmin<stripClosest(temp, j, d).dmin)
        return d;
    else
        return stripClosest(temp, j, d);
}
